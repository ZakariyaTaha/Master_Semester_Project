...
###############################################################################
def nodes_near_point(x, y, z, kdtree, kd_idx_dic, x_coord='x', y_coord='y', z_coord='z',
                     n_neighbors=-1,
                     radius_m=150,
                     verbose=False):
    """
...
###############################################################################
def G_to_kdtree(G_, x_coord='x', y_coord='y', z_coord='z', verbose=False):
    """
...
def _get_graph_extent(G_):
    ...
    zall = [G_.nodes[n]['z'] for n in G_.nodes()]
    xmin, xmax = np.min(xall), np.max(xall)
    ymin, ymax = np.min(yall), np.max(yall)
    zmin, zmax = np.min(zall), np.max(zall)
...
###############################################################################
def _get_node_positions(G_, x_coord='x', y_coord='y', z_coords='z'):
    ...
    for i, n in enumerate(G_.nodes()):
        n_props = G_.nodes[n]
        x, y, z = n_props[x_coord], n_props[y_coord], n_props[z_coords]
        arr[i] = [x, y, z]
    return arr

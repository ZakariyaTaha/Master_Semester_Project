        ...
        for iteration in range(self.starting_iteration, self.starting_iteration+iterations+1):
            ...
            if self.print_every is not None:
                if iteration%self.print_every==0:
                    ...
                    loss_v1 = train_step_results["loss1"] if "loss1" in train_step_results.keys() else None
                    loss_v12 = train_step_results["loss2"] if "loss2" in train_step_results.keys() else None
                    loss_v13 = train_step_results["loss3"] if "loss3" in train_step_results.keys() else None
                    to_print = "[{:0.0f}min][{:0.2f}s] - Iteration: {} (Train-batch Loss: {:0.6f}, {:0.6f}, {:0.6f})"
                    to_print = to_print.format(elapsed_time, block_iter_elapsed_time, iteration, loss_v1, loss_v12, loss_v13)

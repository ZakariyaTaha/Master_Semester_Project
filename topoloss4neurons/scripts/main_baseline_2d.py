...
class TrainingStep(object):
    ...
    def __call__(self, iterations, network, optimizer, lr_scheduler, loss_function):
        ...
        f = []
        s = np.sqrt(self.crop_size[0]**2+self.crop_size[1]**2)
...
def process_dataset(dp, in_channels):
    ...
    label = np.zeros_like(dilated, np.uint8)
    label[dilated>0] = IGNORE
    label[dp.label_thin.copy()>0] = 1        
    '''
    label = dp.label_thin
    
    return ExtDataPoint(image, label)
...
def main(config_file="main.config"):
    ...
    dataset_training = load_dataset(dataset, "training", size="orig", labels="orig", each=1)
    logger.info("Done. {} datapoints loaded.".format(len(dataset_training)))

    logger.info("Loading validation dataset '{}'...".format(dataset))
    dataset_validation = load_dataset(dataset, "testing", size="orig", labels="orig", each=1)
    ...
    if batch_size>len(dataset_training):
        ...
        logger.info("Batch-size is larger than the dataset! We set batch_size from {} to {}".format(batch_size, new_batch_size))

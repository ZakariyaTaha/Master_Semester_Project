...
ExtDataPoint = namedtuple("ExtDataPoint", ['image', 'label', 'label_ignore'])
...
class TrainingStep(object):
    ...
    def __call__(self, iterations, network, optimizer, lr_scheduler, loss_function):
        ...
        images  = np.transpose(np.float32(images)/255.0, (0,4,1,2,3))
    ...
    def __call__(self, iteration, network, loss_function):
        ...
        with nt.torch_no_grad:
            for i,dp in enumerate(self.dataset):
                ...
                image_i = np.transpose(np.float32(dp.image[None])/255.0, (0,4,1,2,3))
                label_i = np.int64(dp.label_ignore[None])
                weights_i = None
                ...
                corr, comp, qual = tl.correctness_completeness_quality(skeletonize_3d(pred_mask>0), 
                                                                       dp.label, slack=5, 
                                                                       eps=1e-12)
                ...
                if False:
                    ...
                    tifffile.imsave(os.path.join(output_valid, "pred_{:06d}_iter{:06d}.tiff".format(i, iteration)), pred_np)
...
def process_dataset(dp, in_channels):
    ...
    label_ignore[dp.label_thin>0] = 1
        
    return ExtDataPoint(image, dp.label_thin, label_ignore)
...
def main(config_file="main.config"):
    ...
    dataset_training = load_dataset(dataset, "training", size="orig", labels="thin", each=1)

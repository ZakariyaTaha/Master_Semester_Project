...
from topoloss4neurons_mod.retracing.retracing_routines import resamplePaths
    ...
    def __call__(self, iteration, network):
        ...
        with nt.torch_no_grad:
            for i,dp in enumerate(self.dataset):
                ...
                new_label_i = resamplePaths(new_label_i, prob_i, 
...
class TrainingStep(object):
    ...
    def __call__(self, iterations, network, optimizer, lr_scheduler, loss_function):
        ...
        f = lambda image: nt.random_intensity_remap(image, 0.0)
        ...
        with nt.torch_no_grad:
            for i,dp in enumerate(self.dataset):
                ...
                if True:
                    ...
                    tifffile.imsave(os.path.join(output_valid, "pred_{:06d}_iter{:06d}.tiff".format(i,iteration)), prob)
...
# CUDA_VISIBLE_DEVICES=3 python main_retracing_3d.py -c main_retracing_3d.config
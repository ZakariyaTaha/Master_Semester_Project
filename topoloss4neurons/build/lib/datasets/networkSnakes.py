...
class snakes_3D:
    ...
    def upload_surface(self, surface, surface_dist, surface_phi, surface_theta):
        self.surface = surface
        self.surface_dist = surface_dist
        self.surface_phi = surface_phi
        self.surface_theta = surface_theta
...
class network_snakes_3D:
    ...
    def upload_surface(self, surface):
        self.surface = surface.astype(np.float32)
        ...
        for s in self.snakes:
            s.upload_surface(self.surface, self.surface_dist, self.surface_phi, self.surface_theta)
...
def computeA(non, alpha=0, beta=0):
    A = np.zeros((non,non))

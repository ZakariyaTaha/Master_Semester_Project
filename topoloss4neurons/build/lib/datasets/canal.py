...
# ===================================================================================
base = "/cvlabdata2/home/oner/Canals/trainingImgRiau/"
# base_graphs = "/cvlabdata2/cvlab/datasets_leo/isbi12_em/graphs_isbi12/lbl_graph/train"
path_images={"train":os.path.join(base, 'train'),
...
train_names = sorted([x for x in os.listdir(os.path.join(base, path_images["train"])) if x.startswith("train")])
test_names = sorted([x for x in os.listdir(os.path.join(base, path_images["test"])) if x.startswith("test")])
...
def _data_point(fid, size="train", labels="all", dist_lbl=True, graph=False, dist_thresh=20):
    ...
    label_thin = np.load(os.path.join(path_images[size], "distlbl" + basename[-5] + ".npy"))

...
class conv3DBatchNorm(nn.Module):
    def __init__(
        ...
        conv_mod = nn.Conv3d(int(in_channels),
                             int(n_filters),
                             kernel_size=k_size,
                             padding=padding,
                             stride=stride,
    ...
    def __init__(self, in_size, out_size, is_norm, is_groupnorm=True):
        ...
        if is_norm:
            self.conv1 = nn.Sequential(
                nn.Conv3d(in_size, out_size, 1, 1, 0),
                # nn.BatchNorm2d(out_size),
                nn.GroupNorm(min(8, out_size), out_size) if is_groupnorm else nn.BatchNorm3d(out_size),
                # nn.ReLU(),
            )
        else:
            self.conv1 = nn.Sequential(nn.Conv3d(in_size, out_size, 1, 1, 0))
...
# class unetConv22(nn.Module):
#     def __init__(self, in_size, out_size, is_norm, is_groupnorm=True):
#         super(unetConv22, self).__init__()
...
class unetUp(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        padding = 2 * [offset // 2, offset // 2, offset // 2] #### Not sure
    ...
    def __init__(self, in_size, out_size, is_norm, is_groupnorm=True):
        ...
        if is_norm:
            self.conv1 = nn.Sequential(
                ...
                nn.Conv3d(in_size, in_size*er, 1, 1, 1),
                nn.Conv3d(in_size*er, out_size * er, 3, 1, 0),
                nn.Conv3d(out_size*er, out_size, 1, 1, 0),
                # nn.BatchNorm2d(out_size),
                nn.GroupNorm(min(8, out_size), out_size) if is_groupnorm else nn.BatchNorm3d(out_size),
                nn.ReLU(),
            )
            self.conv2 = nn.Sequential(
                ...
                nn.Conv3d(out_size, out_size * er, 1, 1, 1),
                nn.Conv3d(out_size * er, out_size * er, 3, 1, 0),
                nn.Conv3d(out_size * er, out_size, 1, 1, 0),
                # nn.BatchNorm2d(out_size),
                nn.GroupNorm(min(8, out_size), out_size) if is_groupnorm else nn.BatchNorm3d(out_size),
                nn.ReLU(),
...
class unetUp_expand(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, is_batchnorm=False):
        ...
        self.conv = unetConv3_expand(in_size, out_size, is_batchnorm)
        if is_deconv:
            self.up = nn.ConvTranspose3d(in_size, out_size, kernel_size=2, stride=2)
        else:
            self.up = nn.UpsamplingBilinear3d(scale_factor=2)
...
class segnetDown2(nn.Module):
    def __init__(self, in_size, out_size):
        ...
        self.maxpool_with_argmax = nn.MaxPool3d(2, 2, return_indices=True)
...
class residualBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, n_filters, stride=1, downsample=None):
        ...
        self.convbn1 = nn.Conv3DBatchNorm(in_channels, n_filters, k_size=1, bias=False)
        self.convbn2 = nn.Conv3DBatchNorm(
            n_filters, n_filters, k_size=3, padding=1, stride=stride, bias=False
        )
        self.convbn3 = nn.Conv3DBatchNorm(
...
class linknetUp(nn.Module):
    def __init__(self, in_channels, n_filters):
        ...
        # B, C/2, H, W -> B, C/2, H, W
        self.deconvbnrelu2 = nn.deconv3DBatchNormRelu(
...
class FRRU(nn.Module):
    ...
    def __init__(self, 
        ...
        self.conv_res = nn.Conv3d(out_channels, 32, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, y, z):
        ...
        upsample_size = torch.Size([_s * self.scale for _s in y_prime.shape[-3:]]) #### Not sure
...
class multiResolutionFusion(nn.Module):
    def __init__(self, channels, up_scale_high, up_scale_low, high_shape, low_shape):
        ...
        self.conv_high = nn.Conv3d(high_shape[1], channels, kernel_size=3)

        if low_shape is not None:
            self.conv_low = nn.Conv3d(low_shape[1], channels, kernel_size=3)
...
class chainedResidualPooling(nn.Module):
    def __init__(self, channels, input_shape):
        ...
        self.chained_residual_pooling = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.MaxPool3d(5, 1, 2),
            nn.Conv3d(input_shape[1], channels, kernel_size=3),
        )
    ...
    def forward(self, x):
        ...
        if self.training or self.model_name != "icnet":  # general settings or pspnet
            ...
            for pool_size in self.pool_sizes:
                k_sizes.append((int(h / pool_size), int(w / pool_size), int(d / pool_size)))
                strides.append((int(h / pool_size), int(w / pool_size), int(d / pool_size)))
...
def get_interp_size(input, s_factor=1, z_factor=1):  # for caffe
    ori_h, ori_w, ori_d = input.shape[2:]
    ...
    resize_shape = (int(ori_h), int(ori_w), int(ori_d))
    return resize_shape


def interp(input, output_size, mode="bilinear"):
    n, c, ih, iw, ide = input.shape
    oh, ow, od = output_size

    # normalize to [-1, 1]
    h = torch.arange(0, oh) / (oh - 1) * 2 - 1
    w = torch.arange(0, ow) / (ow - 1) * 2 - 1
    d = torch.arange(0, od) / (od - 1) * 2 - 1

    grid = torch.zeros(oh, ow, od,2)
    grid[:, :, 0] = w.unsqueeze(0).repeat(oh, 1)
    grid[:, :, 1] = h.unsqueeze(0).repeat(ow, 1).transpose(0, 1) #### Not sure
